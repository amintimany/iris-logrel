...
Section Stack_refinement.
  ...
  Proof.
    ...
    iVs (step_tlam _ _ j K with "[Hj]") as "Hj"; eauto.
    ...
    iVsIntro. iApply (wp_bind [AppRCtx (RecV _)]);
      iApply wp_wand_l; iSplitR; [iIntros (v) "Hv"; iExact "Hv"|].
    ...
    iVs (own_alloc (● (∅ : stackUR))) as (γ) "Hemp"; first done.
    ...
    iVs (stack_owns_alloc ⊤ _ _ _ with "[Hoe Histk]") as "[Hoe Hls]".
    { by iFrame "Hoe Histk". }
    ...
    iVs (inv_alloc stackN with "[Hinv]") as "#Hinv"; trivial.
    { iNext; iExact "Hinv". }
    clear istk.
    Opaque stack_owns.
    (* splitting *)
    ...
    - iExists (_, _), (_, _); iSplit; eauto. iSplit.
      + (* refinement of push *)
        ...
        iIntros "{$Hheap $Hstk} !> Hstk !==>".
        iVs ("Hclose" with "[Hoe Hstk' HLK Hl Hstk]").
        ...
        iFrame "Hheap". iNext. iIntros (ltmp) "Hltmp". iVsIntro.
        ...
        * (* CAS succeeds *)
          ...
          iVs (steps_CG_locked_push _ _ j K _ _ _ _ _ with "[Hj Hl Hstk']")
            as "[Hj [Hstk' Hl]]".
          { rewrite CG_locked_push_of_val. by iFrame "Hspec Hstk' Hj". }
          iApply (wp_cas_suc _ _ _ _ _ _ _ _ _ _); simpl; trivial.
          iIntros "{$Hheap $Hstk} !> Hstk !==>".
          iVs ("Hclose" with "==>[-Hj]").
          { iVs (stack_owns_alloc with "[Hoe Hltmp]") as "[Hoe Hpt]".
            { by iFrame "Hoe". }
            iVsIntro. iNext. iExists ltmp, _, _.
      ...
      + (* refinement of pop *)
        ...
        * (* The stack is empty *)
          iVs (steps_CG_locked_pop_fail _ _ _ _ _ _ _ with "[Hj Hstk' Hl]")
            as "[Hj [Hstk' Hl]]".
          { by iFrame "Hspec Hstk' Hl Hj". }
          iVsIntro.
          iVs ("Hclose" with "[-Hj Hmpt]").
          { iNext. iExists _, _, _. by iFrame "Hoe Hstk' Hstk Hl". }
          iApply (wp_bind [AppRCtx (RecV _)]);
            iApply wp_wand_l; iSplitR; [iIntros (w) "Hw"; iExact "Hw"|].
          iApply wp_fold; simpl; auto using to_of_val.
          iNext. iVsIntro. iApply wp_rec; auto using to_of_val. iNext. asimpl.
          clear h.
          iApply (wp_bind [AppRCtx (RecV _)]);
            iApply wp_wand_l; iSplitR; [iIntros (w) "Hw"; iExact "Hw"|].
          ...
          iIntros "!> Histk !==>". iVs ("Hclose" with "[-Hj]").
        ...
        * (* The stack is not empty *)
          iVsIntro. iVs ("Hclose" with "[-Hj Hmpt HLK']").
          ...
          -- (* CAS succeeds *)
            ...
            iDestruct "HLK'" as (yn1 yn2 zn1 zn2)
             ...
             iVs (steps_CG_locked_pop_suc _ _ _ _ _ _ _ _ _
                   with "[Hj Hstk' Hl]") as "[Hj [Hstk' Hl]]".
             { by iFrame "Hspec Hstk' Hl Hj". }
             iVsIntro. iVs ("Hclose" with "[-Hj]").
             ...
             iApply wp_fst; [simpl; by rewrite ?to_of_val |
                             simpl; by rewrite ?to_of_val |].
             iNext. iVsIntro. iApply wp_value; simpl.
          ...
          -- (* CAS will fail *)
            ...
            iIntros "{$Hheap $Hstk} !> Hstk !==>". iVs ("Hclose" with "[-Hj]").
    ...
    - (* refinement of iter *)
      ...
      iVs (step_rec _ _ _ _ _ _ _ _ _ with "[Hj]") as "Hj".
      { by iFrame "Hspec Hj". }
      ...
      iApply (wp_bind [AppRCtx _]); iApply wp_wand_l;
        iSplitR; [iIntros (w) "Hw"; iExact "Hw"|].
      iInv stackN as (istk3 w h) "[Hoe [>Hstk' [>Hstk [HLK >Hl]]]]" "Hclose".
      iVs (steps_CG_snap _ _ _ (K ++ [AppRCtx _]) _ _ _ _
      ...
      * rewrite CG_iter_of_val.
        iVs (steps_CG_iter_end _ _ _ _ _ _ with "[Hj]") as "Hj".
        { by iFrame "Hspec Hj". }
        iVsIntro. iVs ("Hclose" with "[-Hj]").
      ...
      * iDestruct "HLK'" as (yn1 yn2 zn1 zn2)
                              "[% [% [#Hrel HLK'']]]"; simplify_eq/=.
        rewrite CG_iter_of_val.
        iVs (steps_CG_iter _ _ _ _ _ _ _ _ with "[Hj]") as "Hj".
        { by iFrame "Hspec Hj". }
        iVsIntro. iVs ("Hclose" with "[-Hj HLK'']").
        { iNext. iExists _, _, _. by iFrame "Hh Hstk' Hstk HLK". }
        simpl.
        ...
        iApply (wp_bind [AppRCtx (RecV _); AppRCtx _]);
          iApply wp_wand_l; iSplitR; [iIntros (w') "Hw"; iExact "Hw"|].
        iApply wp_fst; simpl; rewrite ?to_of_val; trivial. iNext. iVsIntro.
        iApply (wp_bind [AppRCtx (RecV _)]);
          iApply wp_wand_l; iSplitR; [iIntros (w') "Hw"; iExact "Hw"|].
        rewrite StackLink_unfold.
        ...
        replace (CG_iter (of_val f2)) with (of_val (CG_iterV (of_val f2)))
          by (by rewrite CG_iter_of_val).
        iVs (step_snd _ _ _ (K ++ [AppRCtx _]) _ _ _ _ _ _ _ with "[Hj]")
          as "Hj".
...
Theorem stack_ctx_refinement :
  [] ⊨ FG_stack ≤ctx≤ CG_stack : TForall (TProd (TProd
        ...
        (TArrow (TArrow (TVar 0) TUnit) TUnit)).
Proof.
  set (Σ := #[irisΣ lang; authΣ heapUR; authΣ cfgUR; authΣ stackUR]).
